...
public class DIv4_886_C {
    ...
    public static void main(String[] args) {
        ...
        for (int t = 1;t<=test;t++){
            ...
            if(s1.charAt(ind)!='.'){
                ans=ans+""+s1.charAt(ind);
            }
            if(s2.charAt(ind)!='.'){
                ans=ans+""+s2.charAt(ind);
            }
            if(s3.charAt(ind)!='.'){
                ans=ans+""+s3.charAt(ind);
            }
            if(s4.charAt(ind)!='.'){
                ans=ans+""+s4.charAt(ind);
            }
            if(s5.charAt(ind)!='.'){
                ans=ans+""+s5.charAt(ind);
            }
            if(s6.charAt(ind)!='.'){
                ans=ans+""+s6.charAt(ind);
            }
            if(s7.charAt(ind)!='.'){
                ans=ans+""+s7.charAt(ind);
            }
            if(s8.charAt(ind)!='.'){
                ans=ans+""+s8.charAt(ind);
            }
